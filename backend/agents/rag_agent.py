...
class RegRagAgent:
    """
    Main RAG agent for regulatory compliance queries.
    ...
    def __init__(self):
        """
        Initialize the RegRAG Agent with OpenAI and Qdrant clients.

        Raises:
            ValueError: If required API keys or configuration are missing
        """
        logger.info("Initializing RegRAG Agent...")
        ...
        try:
            ...
            self.collection_name = "regulatory-docs"

        except Exception as e:
            logger.error(f"Failed to initialize RegRAG Agent: {e}")
            raise
